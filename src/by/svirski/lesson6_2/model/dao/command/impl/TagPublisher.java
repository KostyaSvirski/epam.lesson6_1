...
public class TagPublisher implements TagActionCommand {

	private final static String FIND_PUBLISHER_SQL = "SELECT NameOfBook, Authors, Genre, DateOfPublish,"
			+ "PublishHouse FROM books WHERE PublishHouse = ?";
	...
	public List<CustomBook> find(String parameters, Connection cn) throws CustomCommandException {
		...
		if (validator.validate(parameters)) {
			try {
				pr = cn.prepareStatement(FIND_PUBLISHER_SQL);
